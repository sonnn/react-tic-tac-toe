...
(
  <div style={aboutStyle}>
    ...
    <h2>Best Practises, Design desicions and patterns</h2>
    <h3>Centralize state</h3>
    <p>One of React's best practises (especially for medium and larger projects) is to keep the entire state of the app minimal and "centralize" (everything is easier if you follow the <a href="https://facebook.github.io/flux/" target="_blank">flux architecture</a> and it's one-way data flow). This app is using <a href="https://mobx.js.org/" target="_blank">Mobx</a> for state management. Mobx is very simple and scalable state manager with a very minimal API and concepts to follow. It's main concept is that you have an <strong>observable state</strong>, you mutate it using <strong>actions</strong> and finally Mobx triggers <strong>reactions</strong> and Reacts "listens" to them and re-renders the app. You can read and learn more about Mobx from an <a href="http://tsevdos.github.io/presentations/react-mobx-2017/" target="_blank">internal presentation</a> I gave for airtickets.gr.</p>
    <p>In our our app now, there is only one store (<code>GameStore.js</code>) that encapsulates the entire store and logic of our state. What I like in Mobx is that you keep everything in pure Javascript. If you take a look at the class you can easily spot the observable state and the actions that mutate it. A minor thing that I don't like in Mobx is the use of <strong>decorators</strong>, which is a neccessary evil if you use ES6 classes. There is an ES5 alternative API, but again it dosn't look so good, so I guess the decorators are the way to go (actually Mobx team suggest to use them).</p>
    <p>So, the entire store and logic is located into a single class (<code>GameStore.js</code>) and as you can see I'm only creating a single instance (singleton) to generate the store (<code>const gameStore = new GameStore()</code>) and immidiately export it (<code>export default gameStore</code>) to the our app. As you can see we use mobx-react tools (a library that gives us react specific toolsfor mobx) to create a <code>Provider</code> that exposes our store to the app (have a look at the <code>index.js</code>). With this technique we can inject our store wherever we want in our app.</p>
    <p>The store except from the observable state and actions, it also contains normal methods for selecting players, checking the board etc. Because the project is small, I kept everything in one class (and file of course). If the app grows bigger we can esily split the class into multiple other classes (for example Board, Position, Markers, etc.) with their own state, methods and logic. If you want to have a look on such an implementation, please have a look at my latest personal react project <a href="https://github.com/tsevdos/repocompare" target="_blank">Repocompare</a>.</p>

    <h3>Smart and presentational (stateless) components</h3>
    <p>Another best practise for React apps is to keep your logic to containers (aka. smart components) and then just render everything else using properties passed down to presentational components (again you can read more from my latest <a href="http://tsevdos.github.io/presentations/react-mobx-2017/index.html#/3/1" target="_blank">presentation</a>). Some of the benefits from this pattern are listed below:</p>
    <ul>
    ...
    <p>The entire app is using only one smart component, the <code>AppContainer.jsx</code>. We inject our store to this component (again using the mobx-react library) and at the same time we "observe" it so everythime we mutate the state react will re-render it. This component sets our main UI methods (<code>setMarkersToPlayers(), markPosition() and resetGame()</code>) and passes them down as properties to our presentational components. Finally, our presentational components (located into <code>components directory</code>), are just recieving everything as properties and render the UI, no magic at all here.</p>
    <p>Another, not so used pattern that I like and use on my react projects, is the "layouts" components. These components are presentational components with a main difference, they act as layouts. This app contains only a single layout component (<code>MainLayout</code>), but if for example we had a router with a sidebar and no footer, we could just add another layout here, pass it to our <code>routes</code> and it will be ready. This pattern might fade since the new version of react router gives a lot of flexibility, but for the current version it make sence to me to keep the "layout" related components there.</p>
